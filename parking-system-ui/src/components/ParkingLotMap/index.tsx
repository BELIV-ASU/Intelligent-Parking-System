...
const ParkingLotMap: React.FC<IParkingLotMap> = ({
  locationDetails,
  setLocationDetails,
}) => {
  ...
  useEffect(() => {
    const loader = new Loader({
      // TODO: remove this api key usage to env variables
    ...
    axios.get('https://xm5kidlp3k.execute-api.us-east-2.amazonaws.com/default/spots')
      .then((response) => {
        if(response.status === 200) {
          let spotsData = response.data;
          spotsData = spotsData.filter(function(parkingSpot:ISpotsData) {
            return parkingSpot.vacant === 'Yes';
          })
          spotsData = spotsData.map((spotCords:ISpotsData) => {
            return {lat: spotCords.lat, lng: spotCords.long};
          })
          loader.importLibrary("maps").then(({ Map }) => {
            initMap(Map, spotsData);
          });
        }
        else {
          throw response;
  ...
  const initMap = (Map: typeof google.maps.Map, spotsData: any[]) => {
    const map = new Map(document.getElementById("google-map")!, {
      // Center as SIM Building
    ...
    if (navigator.geolocation) {
      navigator.geolocation.watchPosition(
        (position) => {
          ...
          // Update map only if user changed location
      ...
      alert("Live tracking not supported or possible. Please contact support");
    }
  ...
  const calculateAndDisplayRoute = async (
    ...
    if (searchParams.size === 0) {
      ...
      distanceMatrixService.getDistanceMatrix(request).then((response) => {
        if (response != null) {
          const parkingSpotDistances = response.rows[0].elements;
          console.log(response)
          const nearestParkingSpot = parkingSpots[parkingSpotDistances.reduce(
            (prev, curr, index) => {
              return prev.distance.value < curr.distance.value
                ? prev
                : { distance: curr.distance, index };
            },
            { distance: { value: Infinity }, index: 0 }
          ).index];
          console.log(nearestParkingSpot);
          directionService.route(
            {
              origin: new google.maps.LatLng(userPosition.lat, userPosition.lng),
              destination: new google.maps.LatLng(
                nearestParkingSpot.lat,
                nearestParkingSpot.lng
              ),
              travelMode: "DRIVING",
            },
  ...
  return (
    <div>
      <Box id="google-map" style={isMobileView ? useStyles.mapStylesMobile : useStyles.mapStylesWeb}></Box>
    </div>
  );
};
